...
public class TimeLimitTest {
    ...
    @Test
    public void shouldReturnImmedietly() {
        long time = System.currentTimeMillis();
        HTTP.Response response = HTTP.POST(neo4j.httpURI().resolve("/v1/service/query").toString(),
                QUERY_MAP0);
        // Using 500ms as a proxy for immedietly
        assertTrue(System.currentTimeMillis() - time < 500L);
    ...
    public void shouldReturnOneRecords() {
        ...
        HTTP.Response response = HTTP.POST(neo4j.httpURI().resolve("/v1/service/query").toString(),
                QUERY_MAP1);
        assertTrue(System.currentTimeMillis() - time <= 1000L);
        ArrayList actual = response.content();
    ...
    public void shouldReturnDefaultRecords() {
        ...
        HTTP.Response response = HTTP.POST(neo4j.httpURI().resolve("/v1/service/query").toString(),
                QUERY_MAP2);
        assertTrue(System.currentTimeMillis() - time < DEFAULT_TIME_LIMIT);
        ArrayList actual = response.content();
        assertArrayEquals(EXPECTED_DEFAULT.toArray(), actual.toArray());
    }
